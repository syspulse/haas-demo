...
case class Circulating(timestamp:Option[Long],totalSupply:BigInt,circulatingSupply:BigInt,inflation:Double,locks:List[Lock],totalHolders:Long,topHolders:List[Holder],tokenId:Option[String] = None)
...
object CirculatingJson extends JsonCommon {
  ...
  implicit val jf_c = jsonFormat8(Circulating.apply _)
}
