...
class PipelineCryptoCompTerse(feed:String,output:String)(implicit config:Config) 
  extends PipelineCryptoComp[CryptoCompTerse](feed,output) {

  import CryptoCompJson._

  override def apiSuffix():String = "pricemulti" + super.apiSuffix()

  def parseCryptoComp(data:String):Seq[CryptoCompTerse] = {
    ...
    try {
      if(data.stripLeading().startsWith("{")) {
        try {
          ...
          Seq(CryptoCompTerse(pairs = obj,ts = Some(System.currentTimeMillis)))
        }
        catch {
          case e:Exception => 
  ...
  def parse(data:String):Seq[CryptoCompTerse] = {
  ...
  def transform(cct: CryptoCompTerse): Seq[Price] = {
