...
class CgIngestCoinInfo(config:Config,c:Configuration) extends CgIngest[CoingeckoCoinInfo](config,c) {
  import CoingeckoJson._

  override def url(host:String = "http://localhost:8100") = s"${host}/coin"
