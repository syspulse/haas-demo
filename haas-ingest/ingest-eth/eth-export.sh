...
DOCKER_DEF=${DOCKER_DEF:-649502643044.dkr.ecr.eu-west-1.amazonaws.com/syspulse/ethereum-etl:2.0.3.1}
...
if [ "$DOCKER" != "" ]; then
  case "$DOCKER" in
     "default")
        docker run --rm --name eth-export $DOCKER_DEF $ENTITY $START_BLOCK_ARG $END_BLOCK_ARG --provider-uri $ETH_RPC $OUTPUT $OUTPUT_FILE $EXTRA
        ;;
     *)
